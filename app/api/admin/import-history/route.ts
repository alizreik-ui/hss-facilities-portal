...
async function loadPS(){const parts=await Promise.all([0,1,2,3,4,5].map(i=>text(`${i}.txt`)));const csv=gunzipSync(Buffer.from(parts.join(''),'base64')).toString('utf8');return parseCSV(csv)}
