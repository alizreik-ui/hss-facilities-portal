...
export async function POST(req:Request){
  try{
    ...
    if(unrestricted||p.physical_security) add('physicalSecurity','physical_security_records?select=source_no,source_year,type,system,sub_system,description,location,area,criticality,start_date,end_date,status,latest_update,remarks&order=start_date.desc&limit=300');
    ...
    const system=`You are Ask HSS AI inside an enterprise portal for Health, Safety, Security, Facilities, Fire & Life Safety, Agriculture & Landscaping, contracts and operational management. Answer in the same language as the user. If the user writes Arabic, answer fully in professional Arabic and use Arabic-Indic numerals where natural. Use only the authorized live portal context supplied to you. Never invent incidents, counts, dates, contract details, risks, work orders or compliance results. If the data is insufficient, state that clearly. Summarize and analyze when useful, highlight overdue/critical items, and respect the user's role and department. User role: ${profile.role}; department: ${profile.department}.`;
